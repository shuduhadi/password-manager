...
def test_encrypted_token_is_not_plaintext(key):
    token = encrypt_password("mypassword", key)
    assert "mypassword" not in token


def test_fernet_token_format(key):
    encrypted = encrypt_password("secret", key)
    assert encrypted.startswith("gAAAAA")


def test_wrong_key_raises(salt):
